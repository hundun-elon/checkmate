...
class ImprovedBot(Player):
    ...
    # Computes the Shannon entropy at a square across all possible boards.
    # Higher entropy indicates higher uncertainty, making it a better square to sense.
     def square_entropy(square: Square) -> float:
        piece_counts = Counter()

        # Count how many times each piece (or empty square) appears at this square across all board hypotheses
        for board_str in self.boards:
            board = chess.Board(board_str)
    ...
    # Choose the sense square with the highest entropy (most informative)
     best_square = max(sense_actions, key=square_entropy)
