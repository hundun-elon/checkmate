...
class ImprovedBot(Player):
    ...
    def choose_sense(self, sense_actions: List[Square], move_actions: List[chess.Move], seconds_left: float) -> Square:
        ...
        def get_potential_moves_squares() -> Set[Square]:
            """Get squares that are potential move destinations."""
            move_squares = set()
            for move in move_actions:
                move_squares.add(move.to_square)
                # Also consider squares around the destination
        ...
        def analyze_threat_potential(square: Square) -> float:
            """Analyze how sensing this square might reveal threats."""
            threat_score = 0.0
            ...
            for board_str in self.boards:
                ...
                if piece and piece.color != self.color:  # Enemy piece
                    ...
                    # Pieces that can attack many squares are more threatening
        ...
        def update_sense_history(self, square: Square):
            """Track recently sensed squares to avoid repetition."""
            
            self.recent_sense_history.append(square)
            # Keep only recent history (last 5 senses)
        ...
        def compute_weighted_score(square: Square) -> float:
            ...
            # Bonus for squares near our potential moves
            ...
            # Penalize squares we've recently sensed
            if square in self.recent_sense_history[-3:]:  # Last 3 senses
                score *= 0.7
            elif square in self.recent_sense_history[-5:]:  # Last 5 senses
                score *= 0.7
        ...
        # If we have very few board hypotheses, use simpler entropy
        if len(self.boards) < 10:
            best_square = max(filtered_actions, key=square_entropy)
        else:
            # Use weighted scoring for more sophisticated selection
            best_square = max(filtered_actions, key=compute_weighted_score)
        
        update_sense_history(self, best_square)

        # Debug information
        # entropy_val = square_entropy(best_square)
        # weighted_score = compute_weighted_score(best_square)
        
        # print(f"[DEBUG] Chose sense square: {best_square} ({chess.square_name(best_square)})")
        # print(f"[DEBUG] Entropy: {entropy_val:.3f}, Weighted Score: {weighted_score:.3f}")
